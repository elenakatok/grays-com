...
| **`CLASSROOM_CALLBACK_SECRET` not yet set in grays Firebase project** | High — needed before any result callbacks work | The secret `CALLBACK_SECRET_GRAYS_COM` is already set in the classroom's Secret Manager. The same plaintext value needs to be set in the grays project: `firebase functions:secrets:set CLASSROOM_CALLBACK_SECRET` (use `--project grays-mygames-live`). Do this before Phase 2 (live negotiation) at the latest. |
